...
- **Server state** → React Query (TanStack Query v5): data fetching, caching,
  background updates. Identical to paperwork-app — fully portable.
...
**hook**. Never call axios directly from a component. Same pattern as
paperwork-app, ported unchanged.
...
1. **Service** — `src/api/services/<entity>Service.ts`. A class that receives the
   ...
   and exports a pre-built singleton as the default. Methods wrap axios calls in
   try/catch and throw an `Error` with a Dutch, user-safe message. One file per
   domain entity.
...
3. **Query keys** — add an entry to `src/api/queryKeys.ts` following the existing
   ...
   entity-specific factories only when needed. Reuse existing keys; invalidate
   via the relevant factory rather than inventing a parallel scheme.
...
paperwork-app surfaces errors via `IonToast`. This app has no equivalent yet
— until the first screen establishes a pattern, surface errors however that
screen already does and flag the inconsistency rather than inventing a new
one (see the `add-api-hook` skill).

Until this repo has its own established service/hook pair, read
paperwork-app's `src/api/services/expensesService.ts` +
`src/hooks/useExpenses.ts` end to end as the reference.
